...
public class BillService {
    ...
    public List<Bill> getBillsByCategory(String category) {
        return bills.stream()
            .filter(bill -> bill.getName().toLowerCase().contains(category.toLowerCase()))
            .sorted(Comparator.comparing(Bill::getDueDate))
            .collect(Collectors.toList());
    }
    ...
    // 【胡峻玮新增】统计即将到期账单数
    public long countUpcomingBillsWithinDays(int daysThreshold) {
        LocalDate today = LocalDate.now();
        return bills.stream()
            .filter(b -> !b.getDueDate().isBefore(today) &&
                         b.getDueDate().isBefore(today.plusDays(daysThreshold)))
            .count();
    }
    ...
    public List<Bill> searchBills(String keyword) {
        return bills.stream()
            .filter(bill -> bill.getName().toLowerCase().contains(keyword.toLowerCase()))
            .sorted(Comparator.comparing(Bill::getDueDate))
            .collect(Collectors.toList());
    }
} 
